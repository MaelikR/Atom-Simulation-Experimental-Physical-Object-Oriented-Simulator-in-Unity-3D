...
public class FishDNA
{
    ...
    public static FishDNA GenerateRandom()
    {
        return new FishDNA
        {
            swimSpeed = Random.Range(2f, 4f); // pas en dessous de 1.5f

            turnSpeed = Random.Range(1f, 3f),
            curiosity = Random.Range(1f, 5f),
            fleeDistance = Random.Range(2f, 6f),
            energyEfficiency = Random.Range(0.01f, 0.05f),
